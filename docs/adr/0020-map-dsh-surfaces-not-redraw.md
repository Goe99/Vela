...
Vela 的左侧导航按 Multica 的三组十二项**原样摆好**（后来又加了一项，见下方「后续修订」），但每一格背后的实现分三种：Vela 自己画的、点了跳去 DSH 的、以及灰着占位标注「下一期」的。
...
**十三项，不再是十二项。** 记忆库（Memory）在「工作区」组占了新的一项，理由与代价见 ADR-0024。本 ADR 的其余结论不变：DSH 已有的界面只做入口不重画，每一项都必须有明确归属，置灰的两种原因分得开。
