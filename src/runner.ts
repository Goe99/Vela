...
export class Runner {
  ...
  async dispatch(issueId: string): Promise<BoardResult<DispatchResult>> {
    ...
    const attempt = this.dispatchOnce(issueId).finally(() => {
  ...
  private async dispatchOnce(issueId: string): Promise<BoardResult<DispatchResult>> {
    ...
    // 会话创建是第一个不可撤销的副作用，因此放在全部校验之后。
    const created = await api.sessions.create({
      rpcId: this.deps.newId(),
      payload: {
        cwd: issue.workspace,
        ...(exec.agentPreset === undefined ? {} : { agentPreset: exec.agentPreset }),
      },
    })
    ...
    const entry: InFlight = {
      issueId, runId, sessionId, usage: undefined, timedOut: false, timeout: undefined, grace: undefined,
    }
    this.inFlight.set(sessionId, entry)

    // 权限档位：会话已存在但尚未跑任何 turn，是施加档位的正确窗口。
    if (exec.sandbox !== undefined) {
      const applied = this.applySandbox(sessionId, exec.sandbox)
    ...
    const prompted = await api.sessions.prompt({
      rpcId: this.deps.newId(),
      payload: { sessionId, mode: 'queue', content: [{ type: 'text', text: buildPrompt(issue) }] },
    }).catch((error: unknown) => ({
      rpcId: '',
  ...
  observe(sessionId: string, event: SessionEventLike): void {
    ...
    void this.settle(sessionId, outcome, failure)
  }
  ...
  private async requestCancel(entry: InFlight, reason: 'aborted' | 'timeout'): Promise<void> {
    ...
    entry.grace = this.deps.setTimer(() => {
      void this.settle(entry.sessionId, reason, `${defaultFailure(reason)}（未在宽限内收到结束事件）`)
    }, CANCEL_GRACE_MS)
  }
 ...
 * 不额外包装指令——Agent 的行为应由 preset 决定，不由 Vela 悄悄注入。
 */
export function buildPrompt(issue: Issue): string {
  const description = issue.description.trim()
  return description.length === 0 ? issue.title : `${issue.title}\n\n${description}`
}
